...
public:
  ...
  // Query execution
  std::unique_ptr<IResultSet> execQuery(const std::string& query,
  ...
  // Batch operations
  bool execBatch(const std::vector<std::string>& commands,
                 const std::vector<std::vector<std::string>>& parameterSets) override;
