...
private:
    ...
    static void addMockUserResponse(rapidjson::Document &response, rapidjson::Document::AllocatorType &allocator)
    {
    ...
    static void addMockOrdersResponse(rapidjson::Document &response, rapidjson::Document::AllocatorType &allocator)
    {
    ...
    static void addMathResponse(const rapidjson::Document &request,
                                rapidjson::Document &response,
                                rapidjson::Document::AllocatorType &allocator)
    {
        const double a = (request.HasMember("a") && request["a"].IsNumber()) ? request["a"].GetDouble() : 0.0;
        const double b = (request.HasMember("b") && request["b"].IsNumber()) ? request["b"].GetDouble() : 0.0;
        const std::string operation = (request.HasMember("operation") && request["operation"].IsString())
...
private:
    [[nodiscard]] rapidjson::Document processRequest(const rapidjson::Document &request) const
    {
        ...
        response.AddMember("serviceId", rapidjson::Value(identity.serviceId.c_str(), allocator), allocator);
        response.AddMember("timestamp", static_cast<int64_t>(
        ...
        else if (command == "info")
        {
            ...
            resultObj.AddMember("service", rapidjson::Value(identity.serviceName.c_str(), allocator), allocator);
            resultObj.AddMember("id", rapidjson::Value(identity.serviceId.c_str(), allocator), allocator);
            resultObj.AddMember("machine", rapidjson::Value(identity.machineName.c_str(), allocator), allocator);
            resultObj.AddMember("version", rapidjson::Value(identity.version.c_str(), allocator), allocator);
            resultObj.AddMember("environment", rapidjson::Value(identity.environment.c_str(), allocator), allocator);
        ...
        else if (command == "echo")
        {
            std::string message = (request.HasMember("message") && request["message"].IsString())
                                      ...
                                      : "";
            response.AddMember("result", rapidjson::Value(("echo: " + message).c_str(), allocator), allocator);
            response.AddMember("status", "success", allocator);
        }
        else
        {
            response.AddMember("error", rapidjson::Value(("Unknown command: " + command).c_str(), allocator), allocator);
            response.AddMember("status", "error", allocator);
