...
public:
  ...
  findAvailableServices(const std::string& capability) const; // healthy + not overloaded

  // Load balancing
  std::string
  selectBestService(const std::string& capability,
  ...
  // Custom filtering
  std::vector<std::string>
  findServices(const std::function<bool(const ServiceIdentity&)>& predicate) const;
};
