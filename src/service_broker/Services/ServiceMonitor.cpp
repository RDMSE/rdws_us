...
void ServiceMonitor::displayContinuous(const int refreshIntervalSeconds) const {
  while (gateway.isRunning()) {
    ...
    std::cout << "\n" << std::string(80, '-') << '\n';
    std::cout << "Press Ctrl+C to stop monitoring (refreshing every " << refreshIntervalSeconds
              << "s)" << '\n';
...
void ServiceMonitor::showServicesByCapability(const std::string& capability) const {
  ...
  std::cout << std::left << std::setw(15) << "Service ID" << std::setw(20) << "Service Name"
            << std::setw(15) << "Machine" << std::setw(8) << "Load %" << std::setw(10) << "Healthy"
            << '\n';
  std::cout << std::string(70, '-') << '\n';

  for (const auto& serviceId : serviceIds) {
    if (const auto* identity = registry.findServiceById(serviceId)) {
      std::cout << std::left << std::setw(15) << serviceId << std::setw(20) << identity->serviceName
                << std::setw(15) << identity->machineName << std::setw(8) << std::fixed
                << std::setprecision(1) << identity->getLoadPercentage() << std::setw(10)
                << (identity->isHealthy() ? "Yes" : "No") << '\n';
    }
  }
}
...
void ServiceMonitor::printGatewayStatus() const {
  ...
  std::cout << "┌─────────────────────────────────────────────────────────────────────────┐"
            << '\n';
  std::cout << "│ Running: " << (status["running"].GetBool() ? "✅ Yes" : "❌ No")
            << std::string(55, ' ') << "│" << '\n';
...
void ServiceMonitor::printConnectionTable() const {
  ...
  std::cout
      ...
      << '\n';
  std::cout
      << "│ FD  │ Type │ Address                     │ Service ID      │ Status   │ Uptime   │"
      << '\n';
  std::cout
      << "├─────┼──────┼─────────────────────────────┼─────────────────┼──────────┼──────────┤"
      << '\n';
  ...
  for (const auto& conn : connections.GetArray()) {
    ...
    std::cout << "│ " << std::left << std::setw(3) << conn["fd"].GetInt() << " │ " << std::setw(4)
              << conn["type"].GetString() << " │ " << std::setw(27) << address << " │ "
              << std::setw(15) << serviceId << " │ " << std::setw(8) << status << " │ "
              << std::setw(8) << uptime << " │" << '\n';
  }
...
void ServiceMonitor::printServiceTable() const {
  ...
  std::cout << "┌─────────────────┬─────────────────┬─────────────────┬────────┬─────────┬─────────"
               "┬─────────┐"
            << '\n';
  std::cout << "│ Service ID      │ Name            │ Machine         │ Load % │ Reqs    │ Errors  "
               "│ Health  │"
            << '\n';
  std::cout << "├─────────────────┼─────────────────┼─────────────────┼────────┼─────────┼─────────"
               "┼─────────┤"
  ...
  for (const auto& service : services) {
    ...
    std::cout << "│ " << std::left << std::setw(15) << serviceId << " │ " << std::setw(15)
              << serviceName << " │ " << std::setw(15) << machineName << " │ " << std::right
              << std::setw(6) << std::fixed << std::setprecision(1) << service.getLoadPercentage()
              << " │ " << std::setw(7) << service.totalRequests << " │ " << std::setw(7)
              << service.errorCount << " │ " << std::left << std::setw(7) << healthStatus << " │"
              << '\n';
...
void ServiceMonitor::printCapabilityIndex() const {
  ...
  for (const auto& [capability, serviceIds] : capabilityMap) {
    ...
    std::cout << "│ " << std::left << std::setw(20) << cap << " │ " << std::right << std::setw(7)
              << serviceIds.size() << " │ " << std::left << std::setw(43) << ids << " │" << '\n';
  }
