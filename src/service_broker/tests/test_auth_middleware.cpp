...
{
    return AuthMiddleware::base64urlEncode(
...
protected:
    ...
    {
        std::string payload = R"({"sub":")" + subject + R"(","iss":")" + kIssuer +
                              R"(","aud":")" + kAudience +
                              R"(","exp":)" + std::to_string(futureExp()) + extra + "}";
...
{
    const std::string token = makeJwt(kSecret,
        R"({"sub":"bob","iss":")" + std::string(kIssuer) + R"(","aud":")" + std::string(kAudience) +
        R"(","exp":)" + std::to_string(futureExp()) + R"(,"role":"admin"})");
...
{
    const std::string badToken = makeJwt("wrong-secret",
        R"({"sub":"alice","iss":")" + std::string(kIssuer) + R"(","aud":")" +
...
TEST_F(JwtTest, ExpiredToken_Returns401)
{
    const std::string token = makeJwt(kSecret,
        R"({"sub":"alice","iss":")" + std::string(kIssuer) + R"(","aud":")" +
        std::string(kAudience) + R"(","exp":)" + std::to_string(pastExp()) + "}");
...
{
    const std::string token = makeJwt(kSecret,
        R"({"sub":"alice","iss":"other-issuer","aud":")" + std::string(kAudience) +
...
{
    const std::string token = makeJwt(kSecret,
        R"({"sub":"alice","iss":")" + std::string(kIssuer) + R"(","aud":"other-aud","exp":)" +
...
TEST_F(JwtTest, NoIssuerCheck_WhenIssuerConfigEmpty)
{
    ...
    const std::string token = makeJwt(kSecret,
        R"({"sub":"charlie","exp":)" + std::to_string(futureExp()) + "}");
...
TEST_F(JwtTest, AudienceArray_Matches)
{
    ...
    const std::string token = makeJwt(kSecret,
        R"({"sub":"alice","iss":")" + std::string(kIssuer) +
        R"(","aud":["other",")" + std::string(kAudience) + R"("],"exp":)" +
        std::to_string(futureExp()) + "}");
    const auto result = auth_->authenticate(makeRequest("/invoke/x", "Bearer " + token));
    EXPECT_TRUE(result.authorized);
}
