...
TEST_F(EventBusTest, Subscribe_ReceivesPublishedEvent) {
  ...
  bus.subscribe("greet",
                [&](const std::string&, const rapidjson::Document&) { received.store(true); });
...
TEST_F(EventBusTest, MultipleSubscribers_AllReceive) {
  ...
  for (int i = 0; i < 3; ++i) {
    bus.subscribe("ping",
                  [&](const std::string&, const rapidjson::Document&) { count.fetch_add(1); });
  }
...
TEST_F(EventBusTest, WildcardSubscription_ReceivesAllTopics) {
  std::atomic<int> count{0};
  bus.subscribe("*", [&](const std::string&, const rapidjson::Document&) { count.fetch_add(1); });
...
TEST_F(EventBusTest, Unsubscribe_NoLongerReceives) {
  ...
  const SubscriptionId id = bus.subscribe(
      "evt", [&](const std::string&, const rapidjson::Document&) { count.fetch_add(1); });
...
TEST_F(EventBusTest, ListTopics_HasSubscribedTopics) {
  ...
  bus.subscribe(
      "*", [](const std::string&, const rapidjson::Document&) {}); // wildcard should not appear
