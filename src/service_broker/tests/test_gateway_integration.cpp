                          ...
                          std::chrono::milliseconds maxWait = std::chrono::milliseconds(3000))
{
    ...
    {
        if (std::filesystem::exists(sockPath)) return true;
...
static bool waitForRegistration(ServiceGateway &gw,
                                std::chrono::milliseconds maxWait = std::chrono::milliseconds(3000))
{
    const auto deadline = std::chrono::steady_clock::now() + maxWait;
    while (std::chrono::steady_clock::now() < deadline)
...
TEST_F(GatewayIntegrationTest, EchoRequest_ServiceResponds_Success)
{
    ...
    client->setRequestHandler([](const rapidjson::Document &req) -> rapidjson::Document {
        ...
        const std::string msg =
            (req.HasMember("message") && req["message"].IsString())
    ...
    const PendingRequest result =
        gw->waitForResponse(requestId, std::chrono::milliseconds(5000));
...
TEST_F(GatewayIntegrationTest, SlowService_WaitForResponse_TimesOut)
{
    ...
    const PendingRequest result =
        gw->waitForResponse(requestId, std::chrono::milliseconds(300));
...
TEST_F(GatewayIntegrationTest, ServiceDisconnects_InFlight_RequestFails)
{
    ...
    client->setRequestHandler([handlerStarted, stopHandler](const rapidjson::Document &) -> rapidjson::Document {
    ...
    *stopHandler = true;
    ...
    const PendingRequest result =
        gw->waitForResponse(requestId, std::chrono::milliseconds(2000));
